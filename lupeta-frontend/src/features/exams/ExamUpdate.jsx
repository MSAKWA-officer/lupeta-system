...
export default function ExamUpdate() {
  ...
  useEffect(() => {
    examsApi
      ...
      .then((res) => {
        ...
        setForm({
          name: ex.name || '',
          term_id: ex.term_id ? String(ex.term_id) : (ex.Term?.id ? String(ex.Term.id) : ''),
          start_date: ex.start_date || '',
          end_date: ex.end_date || '',
          weight_percent: ex.weight_percent ?? 100,
        });
      })
      .catch(() => setError('Failed to load this exam.'))
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to update the exam.');
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="p-4">
      ...
      <div className="mt-3 max-w-xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-6 py-5">
          <h2 className="text-lg font-semibold text-black">Edit Exam</h2>
        </div>

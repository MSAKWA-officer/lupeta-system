...
export default function TeacherUpdate() {
    ...
    teachersApi
      ...
      .then((res) => {
        ...
        setForm({
          ...
          subject_ids: (t.subjectsExpertise || []).map((s) => s.id),
        });
      })
      .catch(() => setError('Failed to load this teacher.'))
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setTimeout(() => navigate(`/dashboard/teachers/${id}`), 1000);
    } catch (err) {
  ...
  return (
    <div className="p-4">
      <Link to={`/dashboard/teachers/${id}`} className="text-sm text-blue-600 hover:underline">
        ← Back
      </Link>

      <div className="mt-3 max-w-2xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-6 py-5">
          <h2 className="text-lg font-semibold text-black">Edit Teacher</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5">
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <Link to={`/dashboard/teachers/${id}`} className="text-sm text-black hover:underline">
              Cancel
            </Link>
          </div>
        </form>
